...
export default class DadalinYaku extends BaseYaku {
    check(tiles: Chess[]): boolean {
        ...
        // 检查是否只包含条子2-8
        const validNumbers = new Set([2, 3, 4, 5, 6, 7, 8]); // 条子2-8
    ...
    generateTiles(count: number): number[] {
        ...
        console.log(`生成大竹林役牌型完成，共${tiles.length}张牌`);
        return tiles;
    }
} 